...
This module is client-only state (no DOM APIs, no SSR issues). The data types (strings, numbers, null) are SSR-safe defaults.
  ...
  <verify>Run `cd /home/lwb3/wallybrain-music && npx svelte-check --tsconfig ./tsconfig.json 2>&1 | tail -20` -- no errors in playerState.svelte.ts</verify>
...
The key architecture: PersistentPlayer is OUTSIDE `{@render children()}`, so SvelteKit never destroys it during client-side navigation.
  ...
  <verify>Run `cd /home/lwb3/wallybrain-music && npx svelte-check --tsconfig ./tsconfig.json 2>&1 | tail -30` -- no type errors. Then `cd /home/lwb3/wallybrain-music && npm run build 2>&1 | tail -10` -- build succeeds.</verify>
