...
This means clicking play on any card in the listing populates the queue with ALL currently visible (potentially filtered) tracks and starts playing from the clicked track's position.
  ...
  <verify>Run `cd /home/lwb3/wallybrain-music && npx svelte-check --tsconfig ./tsconfig.json 2>&1 | tail -20` -- no type errors in TrackCard.svelte or +page.svelte.</verify>
...
5. Build passes. No double audio scenarios.
  ...
  <verify>Run `cd /home/lwb3/wallybrain-music && npx svelte-check --tsconfig ./tsconfig.json 2>&1 | tail -20` -- no errors. Then `cd /home/lwb3/wallybrain-music && npm run build 2>&1 | tail -10` -- build succeeds.</verify>
