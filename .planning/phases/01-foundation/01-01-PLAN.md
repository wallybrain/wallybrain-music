...
<task type="auto">
  ...
  <action>
    Create the SvelteKit project in /home/lwb3/wallybrain-music/ (the project root -- files go directly here, NOT in a subdirectory).

    **Step 1: Scaffold with sv CLI**
    Run `npx sv create . --template minimal --types ts` in /home/lwb3/wallybrain-music/ to scaffold the project in-place. If the CLI prompts interactively, use flags to avoid prompts or manually create the scaffold files.
...
<task type="auto">
  ...
  <action>
    ...
    **Step 2: Create docker-compose.yml**
    ...
    - volumes: bind mount /home/lwb3/wallybrain-music/data:/data (bind mount, not named volume -- simpler for backup per research)
    ...
    **Step 3: Create persistent data directory**
    Create /home/lwb3/wallybrain-music/data/db/, data/audio/, data/peaks/, data/art/ directories on the host. These will be bind-mounted into the container.

    **Step 4: Build and run**
    Run `docker compose build` then `docker compose up -d` in /home/lwb3/wallybrain-music/.
    ...
    **Step 6: Verify database on volume**
    Confirm /home/lwb3/wallybrain-music/data/db/music.db exists on the host filesystem (proving the bind mount works).
  ...
  <verify>
    ...
    6. `ls /home/lwb3/wallybrain-music/data/db/music.db` confirms database file exists on host
    7. `ls /home/lwb3/wallybrain-music/data/audio /home/lwb3/wallybrain-music/data/peaks /home/lwb3/wallybrain-music/data/art` confirms storage directories exist
  </verify>
  <done>
    Docker container builds via multi-stage Dockerfile, starts with docker-compose, passes health checks (hitting /music/health which verifies database connectivity), serves the SvelteKit landing page, and persists data via bind-mounted volume at /home/lwb3/wallybrain-music/data/. Storage directories for audio, peaks, art, and db all exist and are writable.
  </done>
</task>
...
<verification>
1. `docker compose ps` in /home/lwb3/wallybrain-music/ shows container as "healthy"
...
4. SQLite database exists at /home/lwb3/wallybrain-music/data/db/music.db
