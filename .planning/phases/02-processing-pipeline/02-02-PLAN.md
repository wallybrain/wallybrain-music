...
```
cd /home/user/wallybrain-music && docker compose up -d --build
```
...
```
curl -X POST https://wallyblanchard.com/music/api/upload \
  -F "audio=@/home/user/wallybrain-music/package.json"
```
This should return 400 with an error message about unsupported format.
  ...
  <verify>
    ...
    Rebuild and deploy: `cd /home/user/wallybrain-music && docker compose up -d --build`

    Wait for container healthy: `docker inspect --format='{{.State.Health.Status}}' wallybrain-music`

    Test invalid file rejection:
    ```
    curl -s -X POST https://wallyblanchard.com/music/api/upload -F "audio=@/home/user/wallybrain-music/package.json"
    ```
