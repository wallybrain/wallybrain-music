...
Do NOT change `title` from `.notNull()` -- at upload time, derive a default title from the original filename (strip extension). The upload endpoint (Plan 02-02) will handle this.
  ...
  <verify>
    Run `docker build --target builder -t wm-test .` from `/home/lwb3/wallybrain-music/` to verify the builder stage completes (npm packages install, build succeeds).
...
All files must be under `src/lib/server/` to ensure SvelteKit treats them as server-only (never bundled to client).
  ...
  <verify>
    Run `npm run check` (svelte-check) from `/home/lwb3/wallybrain-music/` to verify TypeScript compilation passes for all new files.
