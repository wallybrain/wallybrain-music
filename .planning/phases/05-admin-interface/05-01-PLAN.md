...
<task type="auto">
  ...
  <verify>
    Run `npx svelte-check --tsconfig ./tsconfig.json` in the project root (or inside Docker container) for zero TypeScript errors. Rebuild Docker container with `docker compose -f /home/lwb3/wallybrain-music/docker-compose.yml up -d --build`. Verify:
...
<task type="auto">
  ...
  <verify>
    Run `npx svelte-check --tsconfig ./tsconfig.json` for zero TypeScript errors. Rebuild Docker container with `docker compose -f /home/lwb3/wallybrain-music/docker-compose.yml up -d --build`. Verify:
